...
CATEGORY_CONFIG = {
    "characters": {
        "index": "apkfiles/entries/characters/families/index.json",
        "canonical": "apkfiles/entries/maps/character_order_canonical.txt",
        "order_map": "apkfiles/entries/maps/character_order_map.json",
        "report": "apkfiles/entries/reports/character_order_sync_report.json",
        "key_field": "family",
    ...
    "weapons": {
        ...
        "report": "apkfiles/entries/reports/weapon_order_sync_report.json",
        "key_field": "sourceId",
    ...
    "accessories": {
        ...
        "report": "apkfiles/entries/reports/accessory_order_sync_report.json",
        "key_field": "sourceId",
        "display_fields": ["name", "displayName"],
        "fallback_key_field": "name",
    },
    "bosses": {
        ...
        "report": "apkfiles/entries/reports/boss_order_sync_report.json",
        "key_field": "sourceId",
        "display_fields": ["name", "displayName"],
        "fallback_key_field": "name",
    },
...
def row_key(row: Dict[str, Any], config: Dict[str, Any]) -> str:
    value = row.get(config["key_field"])
    if not value:
...
def build_index_maps(index: Dict[str, Any], config: Dict[str, Any]) -> Tuple[Dict[str, Dict[str, Any]], List[Dict[str, Any]]]:
...
def sync_category(repo: Path, category: str, dry_run: bool) -> Dict[str, Any]:
    ...
    for canon in canonical:
        ...
        ordered.append({
            "key": canon["key"],
            "sourceId": row_id,
            "displayName": canon["displayName"] or matched.get("_displayName") or row_id,
    ...
    report = {
        ...
        "dryRun": dry_run,
        "category": category,
        "seededCanonicalFromCurrentIndex": seeded,
    ...
    if not dry_run:
        ...
        write_json(order_map_path, {
            "schemaVersion": 1,
            "source": f"append-only canonical {category} order",
            "generatedAt": int(time.time()),
            "category": category,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Sync append-only canonical order files for all entry categories.")
