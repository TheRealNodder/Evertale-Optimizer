...
(function(){
  ...
  function setPanel(name,desc){
    const panel=q('#v2SidebarDetailPanel')||q('.v2-description .v2-detail-scroll-panel');
    if(!panel)return;
    panel.textContent='';
  ...
  async function fix(){
    const active=q('#v2SidebarDetailTabs button.active')||q('.v2-detail-tab-btn.active');
    const kind=active?.dataset?.sidebarDetail||active?.getAttribute('data-v2-detail-kind')||'leader';
  ...
  document.addEventListener('click',e=>{if(e.target.closest('#v2SidebarDetailTabs button,.v2-detail-tab-btn,#catalogGrid .unitCard,#v2AwakenTabs button'))later();},true);
