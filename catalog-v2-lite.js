/* catalog-v2-lite.js — live Catalog renderer.
   Virtualization pass: all data stays in memory, only visible cards exist in DOM.
*/
(function(){
  const state={items:[],filtered:[],q:'',type:'all',sort:'newest',token:0,rangeStart:-1,rangeEnd:-1,cols:1,rowHeight:0,raf:0,selectedId:''};
  ...
  function installVirtualCss(){
    if(document.getElementById('catalog-v2-virtual-css'))return;
    const style=document.createElement('style');
    style.id='catalog-v2-virtual-css';
    style.textContent=`
      #catalogGrid[data-virtualized="1"] .v2VirtualSpacer{grid-column:1/-1!important;min-height:0!important;pointer-events:none!important;border:0!important;padding:0!important;margin:0!important;background:transparent!important;box-shadow:none!important;}
      #catalogGrid[data-virtualized="1"] .unitCard{content-visibility:auto;contain:layout paint style;contain-intrinsic-size:420px 620px;}
      @media(max-width:820px){#catalogGrid[data-virtualized="1"] .unitCard{contain-intrinsic-size:360px 470px;}}
    `;
  ...
  function getColumns(){const grid=$('catalogGrid');if(!grid)return 1;const cols=getComputedStyle(grid).gridTemplateColumns.split(' ').filter(x=>x&&x!=='none').length;if(cols>0)return cols;const min=window.innerWidth>=821?336:260;return Math.max(1,Math.floor(grid.clientWidth/min));}
  function estimateRowHeight(){return state.rowHeight||(window.innerWidth>=821?640:500);}
  function measureRow(){const card=document.querySelector('#catalogGrid .unitCard');if(!card)return;const h=Math.ceil(card.getBoundingClientRect().height);if(h>100&&Math.abs(h-state.rowHeight)>16){state.rowHeight=h+16;queueVirtual();}}
  function updateStatus(){const status=$('statusText');if(!status)return;const total=state.filtered.length;const a=total?Math.min(state.rangeStart+1,total):0;const b=Math.min(state.rangeEnd,total);status.textContent=`Showing ${a}-${b} of ${total}`;}
  function queueVirtual(){if(state.raf)return;state.raf=requestAnimationFrame(()=>{state.raf=0;renderWindow();});}
  function render(){const grid=$('catalogGrid');if(!grid)return;state.token++;state.filtered=filter();state.rangeStart=-1;state.rangeEnd=-1;grid.dataset.virtualized='1';grid.innerHTML='';queueVirtual();}
  function renderWindow(){
    const grid=$('catalogGrid');if(!grid)return;
    const total=state.filtered.length;if(!total){grid.innerHTML='';state.rangeStart=0;state.rangeEnd=0;updateStatus();return;}
    const rect=grid.getBoundingClientRect();const pageY=window.scrollY||document.documentElement.scrollTop||0;const gridTop=pageY+rect.top;
    const cols=getColumns();const rowH=estimateRowHeight();const totalRows=Math.ceil(total/cols);
    const overscanRows=window.innerWidth>=821?3:4;
    const scrollTop=Math.max(0,pageY-gridTop);
    const firstRow=Math.max(0,Math.floor(scrollTop/rowH)-overscanRows);
    const lastRow=Math.min(totalRows,Math.ceil((scrollTop+window.innerHeight)/rowH)+overscanRows);
    const start=firstRow*cols;const end=Math.min(total,lastRow*cols);
    if(start===state.rangeStart&&end===state.rangeEnd&&cols===state.cols)return;
    state.rangeStart=start;state.rangeEnd=end;state.cols=cols;
    const topH=Math.max(0,firstRow*rowH);const bottomH=Math.max(0,(totalRows-lastRow)*rowH);
    const top=`<div class="v2VirtualSpacer" style="height:${topH}px"></div>`;
    const mid=state.filtered.slice(start,end).map(renderCard).join('');
    const bottom=`<div class="v2VirtualSpacer" style="height:${bottomH}px"></div>`;
    grid.innerHTML=top+mid+bottom;
    updateStatus();requestAnimationFrame(measureRow);
  }
