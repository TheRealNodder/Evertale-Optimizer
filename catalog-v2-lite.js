...
(function(){
  ...
  function collapseCharacterStates(chars){
    ...
    groups.forEach(list=>{
      ...
      primary.statsByForm=list.flatMap(x=>Array.isArray(x.statsByForm)&&x.statsByForm.length?x.statsByForm:[{stats:x.stats,sourceId:x.sourceId,imageSourceId:x.sourceId,title:x.subtitle,order:x.order}]);
      primary.imageVariants=list.flatMap(x=>Array.isArray(x.imageVariants)&&x.imageVariants.length?x.imageVariants:(Array.isArray(x.images)?x.images.map((url,i)=>({url,image:url,sourceId:x.sourceId,imageSourceId:x.sourceId,title:x.subtitle,stats:x.stats,order:x.order,state:i===0?'base':String(i)})):[]));
      primary.forms=list.flatMap(x=>Array.isArray(x.forms)&&x.forms.length?x.forms:[{sourceId:x.sourceId,imageSourceId:x.sourceId,title:x.subtitle,stats:x.stats,description:x.description,order:x.order}]);
  ...
  function formText(row,key){return row&&typeof row==='object'?row[key]:'';}
  function buildStateRows(item,imgs){
    const variants=Array.isArray(item.imageVariants)?item.imageVariants:[];
    const statsRows=Array.isArray(item.statsByForm)?item.statsByForm:[];
    const descRows=Array.isArray(item.descriptionByForm)?item.descriptionByForm:[];
    const forms=Array.isArray(item.forms)?item.forms:[];
    const max=Math.min(3,Math.max(imgs.length,variants.length,statsRows.length,descRows.length,forms.length,1));
    const rows=[];
    for(let i=0;i<max;i++){
  ...
  function applyState(card,index=0,notify=true){
    if(!card)return null;
    selectCard(card,false);
    const rows=readStateRows(card);
    if(!rows.length)return null;
    const idx=Math.max(0,Math.min(rows.length-1,Number(index)||0));
    const row=rows[idx]||{};
    const img=card.querySelector('.unitThumb img');
    if(img&&row.image&&img.getAttribute('src')!==row.image)img.setAttribute('src',row.image);
    if(img)img.dataset.state=String(idx);
    card.setAttribute('data-duo-index',String(idx));
    card.setAttribute('data-description',row.description||'');
    const title=card.querySelector('.unitTitle');
  ...
  function renderCard(item){
    ...
    const statHtml=statLineHtml(initial.stats);
    ...
    return `<div class="unitCard ${elClass(item.element)}${selected}" data-kind="${item.kind}" data-id="${safe(item.id)}" data-source-id="${safe(item.sourceId)}" data-family="${safe(item.family)}" data-file="${safe(item.file)}" data-order="${safe(item.order)}" data-source-order="${safe(item.order)}" data-file-handle-order="${safe(item.order)}" data-description="${safe(initial.description||'')}" data-state-rows="${attrJson(states)}" data-active-skills="${attrJson(item.activeSkills)}" data-passive-skills="${attrJson(item.passiveSkillDetails)}"><div class="unitLeft"><div class="unitThumb">${img}</div></div><div class="meta"><div class="metaHeader"><div class="metaMain"><div class="nameBlock"><button type="button" class="v2-detail-btn" popovertarget="${detailId}">Details</button><div class="unitName">${safe(item.name)}</div><div class="unitTitle">${safe(initial.title||item.subtitle)}</div>${stateBtns(states)}${duoBtn(item)}</div><div class="chipCol">${chips.join('')}</div></div></div><div class="unitDetails"><div class="statLine">${statHtml}</div>${leader}<div class="skillMini"><button type="button" data-v2-skill="active">Active Skill</button><button type="button" data-v2-skill="passive">Passive Skill</button><button type="button" class="v2-detail-btn" popovertarget="${detailId}">Description</button></div><div class="descriptionPanel" data-descriptions="${attrJson(states)}" hidden>${safe(initial.description||'')}</div></div></div><div id="${detailId}" class="v2-detail-backdrop" popover></div></div>`;
  }
  ...
  function loadedKinds(){return CATEGORIES.filter(k=>Array.isArray(state.raw[k]));}
  function coldKinds(){return CATEGORIES.filter(k=>!Array.isArray(state.raw[k]));}
  ...
  async function loadCategory(category){
    ...
    state.loading[category]=window.EvertaleData.loadEntryCategory(category,category!=='characters').then(rows=>{
      ...
      return state.raw[category];
    }).catch(err=>{
      delete state.loading[category];
      console.warn('[CatalogV2] Category load failed:',category,err);
      state.raw[category]=[];
      return state.raw[category];
    });
    return state.loading[category];
  }
  async function ensureRegistry(){if(state.registry)return state.registry;if(!state.registryPromise)state.registryPromise=loadDuoRegistry().then(registry=>state.registry=registry);return state.registryPromise;}
  function rebuildItems(){
    const entries={characters:state.raw.characters||[],weapons:state.raw.weapons||[],accessories:state.raw.accessories||[],bosses:state.raw.bosses||[]};
  ...
  async function hydrateAll(shouldRender=true){
  ...
  function cardDescription(card){const direct=card?.getAttribute('data-description')||'';if(direct)return direct;try{const rows=readStateRows(card);const row=rows?.[Number(card?.getAttribute('data-duo-index')||0)]||rows?.[0];return row?.description||'';}catch{return'';}}
