...
(function(){
  ...
  function render(){const grid=$('catalogGrid'),status=$('statusText');if(!grid)return;const token=++state.token;state.filtered=filter();state.rendered=0;grid.innerHTML='';if(status)status.textContent=`${state.filtered.length} items shown`;renderMore(token,18);}
  ...
  function cardDescription(card){
    ...
    try{const rows=JSON.parse(decodeURIComponent(card?.querySelector('.descriptionPanel')?.getAttribute('data-descriptions')||''));return rows?.[Number(card?.getAttribute('data-duo-index')||0)]||rows?.[0]||'';}catch{return'';}
  }
  function ensureDetailPopover(card){
    ...
    const img=card.querySelector('.unitThumb')?.innerHTML||'<div class="ph">?</div>';
    const name=card.querySelector('.unitName')?.textContent?.trim()||'Selected';
    const title=card.querySelector('.unitTitle')?.textContent?.trim()||'';
    const skillTop=[card.querySelector('.stateRow')?.outerHTML||'',card.querySelector('.duoFormBtn')?.outerHTML||''].join('');
    const chips=[...card.querySelectorAll('.chipCol .tag')].map(x=>x.outerHTML).join('');
    const stats=card.querySelector('.statLine')?.innerHTML||'';
    const leaderBlock=card.querySelector('.leaderBlock');
    const leader=leaderBlock?leaderBlock.innerHTML:'No leader skill text loaded.';
