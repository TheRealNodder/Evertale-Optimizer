...
(function(){
  const state={items:[],filtered:[],q:'',type:'all',sort:'newest',rendered:0,token:0};
  ...
  function sortRows(rows){
    const copy=[...rows];
    if(state.sort==='az')return copy.sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''))||byNewest(a,b));
    if(state.sort==='za')return copy.sort((a,b)=>String(b.name||'').localeCompare(String(a.name||''))||byNewest(a,b));
    if(state.sort==='oldest')return copy.sort(byOldest);
  ...
  function detailSkillHtml(rows){return (Array.isArray(rows)&&rows.length)?rows.map(s=>`<p><strong>${safe(s.name||s.id||'Skill')}</strong><br>${safe(s.description||'No description loaded.')}</p>`).join(''):'No skills loaded.';}
  ...
  function detailPopover(item,detailId,img,chips,statHtml,leader){
    const skillTop=stateBtns(item.images?.length?item.images:(item.image?[item.image]:[]))+duoBtn(item);
    const desc=safe(item.description||'No description loaded.');
    const leaderHtml=leader||'No leader skill text loaded.';
    return `<div id="${detailId}" class="v2-detail-backdrop" popover><div class="v2-detail-card ${elClass(item.element)}"><div class="unitThumb">${img}</div><div class="v2-detail-name">${safe(item.name)}</div><div class="v2-detail-title">${safe(item.subtitle)}</div><div class="v2-detail-row">${skillTop}</div><div class="v2-detail-row">${chips.join('')}</div>${statHtml?`<div class="v2-detail-stats">${statHtml}</div>`:''}<div class="v2-detail-row">${detailSection('Leader Skill',leaderHtml)}${detailSection('Active',detailSkillHtml(item.activeSkills))}${detailSection('Passive',detailSkillHtml(item.passiveSkillDetails))}${detailSection('Description',desc)}</div></div></div>`;
  }
  function renderCard(item){
    ...
    const detailId=`v2d-${clean(item.id||item.sourceId||item.name)}`;
    return `<div class="unitCard ${elClass(item.element)}" data-kind="${item.kind}" data-id="${safe(item.id)}" data-source-id="${safe(item.sourceId)}" data-family="${safe(item.family)}" data-file="${safe(item.file)}" data-order="${safe(item.order)}" data-source-order="${safe(item.order)}" data-file-handle-order="${safe(item.order)}" data-active-skills="${attrJson(item.activeSkills)}" data-passive-skills="${attrJson(item.passiveSkillDetails)}"><div class="unitLeft"><div class="unitThumb">${img}</div></div><div class="meta"><div class="metaHeader"><div class="metaMain"><div class="nameBlock"><button type="button" class="v2-detail-btn" popovertarget="${detailId}" aria-label="Open details">i</button><div class="unitName">${safe(item.name)}</div><div class="unitTitle" style="display:block!important;visibility:visible!important;opacity:1!important">${safe(item.subtitle)}</div></div>${item.kind==='characters'?stateBtns(imgs):''}${duoBtn(item)}</div><div class="chipCol">${chips.join('')}</div></div><div class="unitDetails">${statHtml?`<div class="statLine">${statHtml}</div>`:''}${leader}<div class="descriptionPanel" data-descriptions="${attrJson(item.descriptionByForm)}" hidden></div></div></div>${detailPopover(item,detailId,img,chips,statHtml,leader)}</div>`;
  }
