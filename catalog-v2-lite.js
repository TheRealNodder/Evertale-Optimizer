...
(function(){
  ...
  function selectCard(card){
    if(!card)return;
    state.selectedId=String(card.getAttribute('data-id')||'');
    if(state.selectedId)window.__EVERTALE_CATALOG_SELECTED_ID=state.selectedId;
    document.querySelectorAll('.unitCard.v2-selected').forEach(c=>c.classList.remove('v2-selected'));
