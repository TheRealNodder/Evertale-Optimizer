/* catalog-v2-lite.js — live Catalog renderer.
   Performance pass: first screen loads fast, extra cards load only on request.
*/
(function(){
  const state={items:[],filtered:[],q:'',type:'all',sort:'newest',rendered:0,token:0,pageSize:24};
  ...
  function ensureMoreButton(){
    const grid=$('catalogGrid');if(!grid)return null;
    let wrap=$('v2LoadMoreWrap');
    if(!wrap){wrap=document.createElement('div');wrap.id='v2LoadMoreWrap';wrap.style.cssText='display:flex;justify-content:center;margin:18px 0 6px;';wrap.innerHTML='<button id="v2LoadMore" class="btn" type="button">Load More</button>';grid.insertAdjacentElement('afterend',wrap);wrap.querySelector('button')?.addEventListener('click',()=>renderMore(state.token,state.pageSize));}
    return wrap;
  }
  function updateStatus(){const status=$('statusText');if(status)status.textContent=`Showing ${state.rendered} of ${state.filtered.length}`;const wrap=ensureMoreButton();if(wrap)wrap.style.display=state.rendered<state.filtered.length?'flex':'none';}
  function render(){const grid=$('catalogGrid');if(!grid)return;const token=++state.token;state.filtered=filter();state.rendered=0;grid.innerHTML='';renderMore(token,state.pageSize);}
  function renderMore(token,count=state.pageSize){const grid=$('catalogGrid');if(!grid||token!==state.token)return;const start=state.rendered,end=Math.min(start+count,state.filtered.length);if(end<=start){updateStatus();return;}grid.insertAdjacentHTML('beforeend',state.filtered.slice(start,end).map(renderCard).join(''));state.rendered=end;updateStatus();}
  ...
  function selectCard(card){if(!card)return;document.querySelectorAll('.unitCard.v2-selected').forEach(c=>c.classList.remove('v2-selected'));card.classList.add('v2-selected');}
