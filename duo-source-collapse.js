...
(function(){
  ...
  function enhanceButtons(){const map=window.EvertaleDuoSource?.formsByRoot;if(!map)return;document.querySelectorAll('.unitCard[data-kind="characters"]').forEach(card=>{const id=card.getAttribute('data-id');const forms=map.get(id);if(!forms||forms.length<2||card.querySelector('.duoFormBtn'))return;card.setAttribute('data-duo-root',id);card.setAttribute('data-duo-index','0');const host=card.querySelector('.stateRow')||card.querySelector('.metaMain')||card.querySelector('.metaHeader')||card.querySelector('.meta')||card;const btn=document.createElement('button');btn.type='button';btn.className='duoFormBtn tag duoSwitchChip';btn.textContent=nextLabel(forms,0);btn.title='Switch to next linked unit';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();let i=(parseInt(card.getAttribute('data-duo-index')||'0',10)+1)%forms.length;updateCard(card,forms[i],id,forms,i,forms.length)});host.appendChild(btn)})}
