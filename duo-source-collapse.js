...
(function(){
  ...
  function updateCard(card,u,root,label,idx,total){if(!card||!u)return;card.setAttribute('data-id',u.id||'');card.setAttribute('data-duo-index',String(idx));const name=card.querySelector('.unitName');if(name)name.textContent=u.name||u.id||'';const title=card.querySelector('.unitTitle');if(title)title.textContent=u.title||u.subtitle||'';const img=card.querySelector('.unitThumb');const url=u.image||(u.imagesLarge&&u.imagesLarge[0]);if(img)img.innerHTML=url?`<img src="${safe(url)}" alt="${safe(u.name||'')}">`:'<div class="ph">?</div>';const desc=card.querySelector('.descriptionText');if(desc)desc.textContent=u.description||u.flavorText||'';const btn=card.querySelector('.duoFormBtn');if(btn)btn.textContent=`${label} ${idx+1}/${total}`}
  function enhanceButtons(){const map=window.EvertaleDuoSource?.formsByRoot;if(!map)return;document.querySelectorAll('.unitCard[data-kind="characters"]').forEach(card=>{const id=card.getAttribute('data-id');const forms=map.get(id);if(!forms||forms.length<2||card.querySelector('.duoFormBtn'))return;const label=forms[0].duoButtonLabel||'Forms';card.setAttribute('data-duo-root',id);card.setAttribute('data-duo-index','0');const host=card.querySelector('.metaMain')||card.querySelector('.metaHeader')||card.querySelector('.meta')||card;const btn=document.createElement('button');btn.type='button';btn.className='duoFormBtn';btn.textContent=label;btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();let i=(parseInt(card.getAttribute('data-duo-index')||'0',10)+1)%forms.length;updateCard(card,forms[i],id,label,i,forms.length)});host.appendChild(btn)})}
