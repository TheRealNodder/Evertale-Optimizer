...
(function(){
  ...
  function imageList(u){const variants=Array.isArray(u?.imageVariants)?u.imageVariants.map(v=>v&&v.url).filter(Boolean):[];const large=Array.isArray(u?.imagesLarge)?u.imagesLarge.filter(Boolean):[];return variants.length?variants:(large.length?large:(u?.image?[u.image]:[]));}
  ...
  function setImages(card,u,idx){const imgs=imageList(u);const imgWrap=card.querySelector('.unitThumb');const img=card.querySelector('.unitThumb img');if(imgWrap){if(imgs.length){imgWrap.innerHTML=`<img src="${safe(imgs[0])}" data-imgs="${safe(encodeURIComponent(JSON.stringify(imgs)))}" data-state="0" alt="${safe(u?.name||'')}">`;}else{imgWrap.innerHTML='<div class="ph">?</div>';}}
    const row=card.querySelector('.stateRow');if(row){if(imgs.length>1){row.setAttribute('data-imgs',encodeURIComponent(JSON.stringify(imgs)));row.innerHTML=imgs.map((_,i)=>`<button type="button" class="stateBtn ${i===0?'active':''}" data-idx="${i}" aria-label="State ${i+1}"></button>`).join('');row.style.display='';}else{row.innerHTML='';row.style.display='none';}}
  }
  ...
  function updateCard(card,u,root,forms,idx,total){if(!card||!u)return;card.setAttribute('data-id',u.id||'');card.setAttribute('data-duo-index',String(idx));setElementClass(card,u);setText(card,'.unitName',u.name||u.id||'');setText(card,'.unitTitle',u.title||u.subtitle||'');setImages(card,u,idx);setChips(card,u);setStat(card,'ATK',statValue(u,'atk'));setStat(card,'HP',statValue(u,'hp'));setStat(card,'SPD',statValue(u,'spd'));setStat(card,'COST',statValue(u,'cost'));const desc=card.querySelector('.descriptionText');if(desc)desc.textContent=u.description||u.flavorText||'';const btn=card.querySelector('.duoFormBtn');if(btn)btn.textContent=nextLabel(forms,idx)}
  function enhanceButtons(){const map=window.EvertaleDuoSource?.formsByRoot;if(!map)return;document.querySelectorAll('.unitCard[data-kind="characters"]').forEach(card=>{const id=card.getAttribute('data-id');const forms=map.get(id);if(!forms||forms.length<2||card.querySelector('.duoFormBtn'))return;card.setAttribute('data-duo-root',id);card.setAttribute('data-duo-index','0');const host=card.querySelector('.chipCol')||card.querySelector('.metaHeader')||card.querySelector('.meta')||card;const btn=document.createElement('button');btn.type='button';btn.className='duoFormBtn tag duoSwitchChip';btn.textContent=nextLabel(forms,0);btn.title='Switch to next linked unit';btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();let i=(parseInt(card.getAttribute('data-duo-index')||'0',10)+1)%forms.length;updateCard(card,forms[i],id,forms,i,forms.length)});host.insertBefore(btn,host.firstChild)})}
  function style(){if(document.getElementById('duoSourceStyle'))return;const s=document.createElement('style');s.id='duoSourceStyle';s.textContent='.duoFormBtn.duoSwitchChip{width:auto;max-width:100%;min-width:0;margin:0;border:1px solid rgba(28,224,154,.45);background:rgba(28,224,154,.14);color:var(--text,#f6f7ff);border-radius:999px;padding:4px 8px;font-size:12px;line-height:1;font-weight:900;cursor:pointer;white-space:normal;text-align:center;overflow-wrap:anywhere}.duoFormBtn.duoSwitchChip:hover{background:rgba(28,224,154,.22)}.unitCard[data-duo-root]{outline:1px solid rgba(28,224,154,.35)}.chipCol .duoFormBtn.duoSwitchChip{align-self:flex-end}body.mobile-detailed .chipCol .duoFormBtn.duoSwitchChip{max-width:96px}body.mobile-compact .chipCol .duoFormBtn.duoSwitchChip{font-size:10px;padding:3px 6px}';document.head.appendChild(s)}
